(function (global) {
  ...
  var CSS =
    ...
    ".heroLive46 .heroLiveBase46{opacity:1!important}" +
    ".heroLive46 .heroLiveBlink46.off,.heroLive46 .heroLiveHair46.off{opacity:0!important}" +
  ...
  function blinkLayerReady(blink) {
    if (!blink) return false;
    if (blink.classList && blink.classList.contains("off")) return false;
    var src = blink.currentSrc || (blink.getAttribute && blink.getAttribute("src")) || "";
    if (!src) return false;
    return (blink.naturalWidth || 0) > 0;
  }
  ...
  function applyPose(nodes, pose) {
    ...
    var look =
      ...
      "deg)";
    if (nodes.sway && nodes.sway.style) nodes.sway.style.transform = sway;
    if (nodes.phys && nodes.phys.style) nodes.phys.style.transform = phys;
    if (nodes.breath && nodes.breath.style) nodes.breath.style.transform = breath;
    if (nodes.look && nodes.look.style) nodes.look.style.transform = look;
    if (nodes.base && nodes.base.style) nodes.base.style.opacity = "1";
    if (nodes.blink && nodes.blink.style) {
      nodes.blink.style.opacity = blinkLayerReady(nodes.blink) && pose.eye < 0.72 ? String(1 - pose.eye) : "0";
    }
    return pose;
  }
  ...
  global.SakurayoLive = {
    ...
    blinkLayerReady: blinkLayerReady,
    applyPose: applyPose,
    attach: attach,
    detach: detach,
    trigger: trigger,
