...
- 四章剧情、Boss 四阶段（75% / 50% / 25%）、主神空间。主神兑换对照第一部概念见 `docs/主神兑换_第一部对照.md`，货架用原创名。核心循环是轮回模板 + 基因锁碎片 + 衍生，存档在 `save.mainGod.core`。大厅已精装修（账户条/八栏/稀有框，含保命防护）。主神 Boss 有四类型独立机制；枪弹或斩击都能打柱/镜核，不刷普通怪。镜像适应只对抄袭者生效。
- 触控摇杆、冲刺、主动技能必须可用。
