...
| D | 寻访卡扩到 16 以内；站桩与大厅背景重出；商店/档案换皮 | **进行中**：风景/CG/残件/武器/证人已换；战斗怪/Boss/宠物、基础·转职·融合静物卡、地图、闪屏、封面和图标已换。正在用免费额度补转职 `full.webp`（升级页还是第四张脸）、技能/职业/融合图标和战斗特效。时装卡和三角色站桩未重出。卡池张数仍未扩到 16 |
| E | 全量 `verify.ps1`、`release/`、APK、CHANGELOG | **未做** |

### 下一步（按优先级）

1. 寻访卡扩到 16 以内。风景、战斗怪/Boss、职业融合静物卡、地图和闪屏已重出；转职升级图/技能图标/特效按队列继续出。不要再刷三角色站桩。看图用 [docs/NAI_GALLERY.md](NAI_GALLERY.md)。
