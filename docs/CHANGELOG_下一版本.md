# 《樱夜·尸潮》下一版本 CHANGELOG

## 主神空间素材与精装修

- 清点后确认：场地、四阶段 Boss、两只虚灵已有图；交互物、大厅店招、四类型徽记和模板图标不够。
- 补 15 张原创 WebP（柱 / 裂口 / 镜核 / 压境纹、大厅、四徽记、五模板、基因锁），不引用原著或外部图。
- 战斗优先画专用交互物，大厅账户条用店招底图和类型徽记，货架 `.ei` 换成分类图标；缺图仍回退通用 stamp / emoji。
- 版本保持 4.6.0。

## 主神漏洞复查

- 气机飞剑也能打柱/镜核（有 0.28s 冷却，最多 8 柄）。
- 转阶段只升级还活着的交互物，已打碎的柱/镜核不再整组复活。
- 转阶段台词按四种监察类型分开；HUD Boss 名跟当前类型走。
- 八栏分类改为可折行，避免手机点不准。测试残留的强制类型会在离开主神时清掉。

## 主神精装修补洞（太刀命中 / 独立 Boss / 保命分类）

- 凛音斩击与技能 AOE 可打碎校验柱、镜核；交互文案改为「枪弹或斩击」。
- 四种主神 Boss 各有专属技能、场地危险和台词；镜像适应只对抄袭者生效。转阶段会升级交互物。
- 外设与模板互补：飞剑改刀势、星屑改过载、磁轨镜改射速、血露瓶改护盾回廊。卡面写重叠提示。
- 因果替身人偶、监管防弹织层、折跃残影靴、虚空避弹符归入「保命防护」栏。
- 交互物与模板光优先画现有 `fx/*.webp` stamp，画不出再回退描边。

## 主神精装修（货架 / 大厅 / Boss 类型）

- 主神货架再加 8 件原创外设：折跃残影靴、磁轨校准镜、血露回廊瓶、飞剑胎记、星屑弹夹、虚空避弹符、监察盲区钥、静默钟摆。只在主神空间生效，旧档补 0。
- 兑换大厅改为玻璃账户条 + 七栏分类（模板 / 基因锁 / 权能 / 血统 / 技法 / 特效），卡面分稀有框和已持有/已装备态。
- 主神 Boss 按通关、层级、深度轮换四种类型：校验者、断线者、抄袭者、压境者。交互物最多 8 个，禁止再刷普通怪。
- 模板与外设补常驻描边特效；结算写明监察类型和交互次数。

## 主神核心循环（模板 / 基因锁 / 衍生）

- 主神大厅新增轮回模板柜：机甲契约、猩红血统、气机内力、星辉弹幕、基因巨兽。主/副槽决定本局身份，只在主神空间生效。
- 通关按层级发放基因锁碎片；可点生存本能、零时差控制、潜能特化。控制锁提供念丝偏转，特化在猩红＋气机时解锁心核激荡。
- 死灵魂火条在有魂火时显示；结算补充模板、碎片、吸血、魂潮、偏转和激荡词条。
- 旧百分比兑换保留作保底。旧存档自动补 `mainGod.core`。大厅重置同时返还模板奖励点和锁碎片。

## 主神兑换大厅（第一部对照）

- 对照《无限恐怖》第一部主线（生化一到神鬼传奇/终战）整理道具与血统，只借鉴高层概念，不照搬原名。
- 主神货架新增 6 项跨轮回兑换：本能解锁序列、镜核原液变异、零时差感应环、轮回队长印、监管防弹织层、裁决投针。
- 仍只在主神空间生效；旧存档缺字段补 0；大厅重置可全额返还。对照表见 `docs/主神兑换_第一部对照.md`。

## 可靠开发基线

- 继续以 `src/index.html` 为唯一源码，未从 `release/` 或旧版本回填实现。
- 新增开发模式错误面板：使用 `?debug=1` 可显示同步脚本异常和未处理 Promise 拒绝，并支持复制、关闭。
- 新增测试模式：使用 `?test=1` 启用固定 60Hz 的确定性 `advanceTime(ms)`、`render_game_to_text` 与受控 `__SAKURAYO_TEST__` API；普通发布模式不暴露测试 API。
- 补回核心 `update(dt)` 契约，保留原 `fixedUpdate(dt)` 实现；真实运行统一经 `scheduleLoop()` 调度。
- 新增可重复 Playwright 冒烟测试，覆盖主菜单、旧存档、三角色、触控、升级、Boss 三阶段、胜负结算和重开。
- 在单文件内部增加 SAVE、DATA、UI、STORY、COMBAT、RENDER、OBSERVABILITY 分区注释，不改变原业务顺序。

## 兼容性与稳定性

- 将 `character`、`skin`、`ownedSkins` 纳入 `sakurayoV3` 默认 schema，旧存档缺字段时合并默认值并保留已有数据。
- `persist()` 写入失败会进入开发错误记录，游戏内存状态仍可继续。
- 为 `CanvasRenderingContext2D.roundRect` 增加矩形回退。
- 将 `String.replaceAll` 改为 `split/join` 等价实现。
- 修正 `AudioContext || webkitAudioContext` 的标识符兼容写法。
- 动态角色卡、关卡卡和升级选项增加稳定 `data-*` 标识。

## 发布

- 源码：`src/index.html`
- 发布文件：`release/樱夜尸潮_下一版本_单文件.html`
- 两者 SHA-256：`0CE75E3EB81B1A922C9A7183E1FE6356ADD89C8E164CF59B7CAFA2335EEA3AEE`
- 源码字节数：1471301

## Image2 角色与 UI 核心美术（2026-07-11）

- 新增月城小夜、神代绫、黑羽凛音三名原创角色的头像、战斗精灵和对话立绘，共 9 个透明 WebP。
- 新增关卡、天赋、剧情、飞升、成就、商店 6 枚无文字导航图标。
- 角色与 UI 使用用户参考图的清爽日系赛璐璐渲染语言，但不复制参考人物身份、服装或构图。
- 新增 15 项资源预加载与状态检查；Android 路径使用 `game/art`，桌面源码使用项目相对路径。
- 菜单角色卡、顶部/HUD 头像、对话立绘、Canvas 战斗角色和底部导航均已接入；图片失败或非默认衣装时自动回退原 SVG/Canvas。
- 后处理新增 alpha 连通域孤岛清理；修复不透明图标 QA 预览的 RGB/RGBA 合成问题。
- Android 同步脚本构建前强制校验 15 项核心资源，防止缺图 APK。
- 最终源码 SHA-256：`93CBDC902F5FF414E438CD112078C39C6BD4739427F45D4101AF05C0D93DA5A4`。
- 最终 APK SHA-256：`9017D2455B3C53DB1E48D56670B5B48D661565DCDF268CD3A6D60DB607AA8CA9`。
