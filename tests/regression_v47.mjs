...
assert.match(index, /const mainGodOpen36=\(\)=>true/);
assert.match(index, /data-mode="mainGod"><b>主神<\/b><small>高难轮回<\/small>/);
assert.match(index, /const unlockedGod = mainGodOpen36\(\)/);
assert.doesNotMatch(index, /通关第4章后解锁主神空间/);
assert.doesNotMatch(index, /unlockedGod = \(save\.done \|\| \[\]\)\.includes\(4\)/);
assert.doesNotMatch(index, /if \(!unlockedGod && pendingMode46 === "mainGod"\)/);
assert.match(read("src/content/packs/maingod-void/pack.js"), /主神空间默认可进/);
