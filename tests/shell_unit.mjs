...
assert.equal(S.version, "4.7.3");
assert.equal(typeof S.cycleBanner, "function");
assert.deepEqual([...S.DRAWERS], ["mission", "mail", "notice", "friend", "calendar", "profile"]);
assert.equal(S.PREVIEW, "本地系统 · 不联网");
...
const crate = S.claimMail(save, "betaCrate");
assert.equal(crate.ok, true);
assert.ok(crate.ticket >= 80);
assert.ok(crate.prism >= 500);
assert.ok(crate.coins >= 25000);
assert.match(code, /icoKind = m\.id === "betaCrate" \? "gift" : "mail"/);
assert.match(code, /id: "betaCrate"/);
...
assert.match(code, /核心、道具、寻访证词都会写入火力/);
...
assert.match(code, /#homeGreet46\{display:none!important\}/);
assert.match(code, /pointer-events:none/);
assert.match(code, /@media\(max-height:430px\)\{html\.landscape46:not\(\.portraitFallback46\) \.shellCal46 button\{min-height:88px/);
