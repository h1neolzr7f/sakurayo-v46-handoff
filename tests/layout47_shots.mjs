...
assert.ok(wishHero && wishHero.w >= 400 && wishHero.w <= 500 && wishHero.h >= 400, `寻访立绘应占左半屏，w=${wishHero?.w} h=${wishHero?.h}`);
...
assert.ok(wishTitle && wishTitle.y >= 36 && wishTitle.y <= 64 && wishTitle.x > 500, `标题应在右上，x=${wishTitle?.x} y=${wishTitle?.y}`);
assert.ok(wishPity && wishPity.y >= 250 && wishPity.y < 340, `保底应贴抽卡钮上方，y=${wishPity?.y}`);
assert.ok(wishDock && wishDock.y > 300, `抽卡坞应贴底，y=${wishDock?.y}`);
assert.ok(wishPity.bottom <= wishDock.y + 8, `保底不得悬在中缝，gap=${wishDock.y - wishPity.bottom}`);
