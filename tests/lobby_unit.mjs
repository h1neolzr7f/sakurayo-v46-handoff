...
assert.match(code, /html\.landscape46 \.wishPity46\{left:auto;right:max\(18px,3%\);top:auto;bottom:max\(112px, calc\(env\(safe-area-inset-bottom, 0px\) \+ 100px\)\)/);
assert.match(code, /html\.landscape46 \.wishHero46\{left:0;top:0;width:48%;height:100%;bottom:0;object-fit:cover/);
...
assert.match(indexSrc, /isTestimonyRun46\(\)\?testimonyMemory38\(\):storyMemory38\(\)/, "证词写 ops.story，演习写 storyChoices38");
assert.match(indexSrc, /这些你都做了。终章会先把最坏的演完。/, "档案回顾要用人话收口");
...
assert.equal(shop.ops.tickets, 0);
assert.equal(shop.ops.picks, 0);
...
assert.ok(Math.abs(allBonusP.crit - 0.06) < 1e-9);
assert.ok(Math.abs(allBonusP.spd - 200 * 1.01) < 1e-6);
assert.ok(Math.abs(allBonusP.bladePower - 1.02) < 1e-9);
assert.ok(Math.abs(allBonusP.skillCd - 10 * 0.99) < 1e-6);
assert.ok(Math.abs(allBonusP.damageReduce - 0.008) < 1e-9);
assert.equal(allBonusP.maxSh, 8);
assert.equal(allBonusP.sh, 8);
assert.ok(Math.abs(allBonusP.dmg - 20 * 1.02 * 1.016) < 1e-6);
assert.ok(Math.abs(allBonusP.maxHp - 102) < 1e-6);
...
assert.ok(Math.abs(jobP.dmg - 20 * 1.005 * 1.01) < 1e-6, "转职按星级加伤：1星 +0.5%，2星 +1%");
...
assert.ok(Math.abs(jobP2.dmg - 20 * 1.025 * 1.01) < 1e-6, "同一张转职五星封顶，按 5×0.5% 加伤");
assert.equal(L.JOB_CARDS.length, 28);
...
assert.ok(Math.abs(fusionP.dmg - 20 * 1.008 * 1.016) < 1e-6, "融合按星级加伤：1星 +0.8%，2星 +1.6%");
...
assert.ok(Math.abs(fusionP2.dmg - 20 * 1.04 * 1.016) < 1e-6, "同一张融合五星封顶，按 5×0.8% 加伤");
...
assert.equal(L.STAR_MAX, 5);
assert.equal(L.starFromCount(0), 0);
assert.equal(L.starFromCount(3), 3);
assert.equal(L.starFromCount(9), 5);
assert.equal(L.starMarks(3), "★★★☆☆");
assert.equal(L.resolveBuildCard({ fusion: "magitech" }).id, "fusion_magitech");
assert.equal(L.resolveBuildCard({ careers: { mech: { formed: 1, branch: "swarm", rank: 2 } } }).id, "job_swarm");
assert.equal(L.resolveBuildCard({ careers: { shrine: { formed: 1, branch: null, rank: 0 } } }).id, "school_shrine");
assert.equal(L.resolveBuildCard({ character: "rion" }).id, "rion_edge");
assert.equal(L.resolveBuildCard({ character: "aya" }).id, "aya_petal");
assert.equal(L.resolveBuildCard({ character: "sayo" }).id, "sayo_echo");
const grantSave = { shop40: L.normalizeOps({}) };
const first = L.grantBuildCard(grantSave, { fusion: "gunshrine" });
assert.equal(first.ok, true);
assert.equal(first.id, "fusion_gunshrine");
assert.equal(first.star, 1);
assert.equal(first.isNew, true);
const second = L.grantBuildCard(grantSave, { fusion: "gunshrine" });
assert.equal(second.star, 2);
...
assert.match(indexHtml, /function hideExplorationLayer41/);
assert.match(indexHtml, /hideExplorationLayer41\(\)/);
assert.match(indexHtml, /html\.landscape46 \.banter\{/);
...
assert.equal(typeof L.exchange, "function");
assert.equal(L.EXCHANGE.shard.coins, 20);
assert.equal(L.EXCHANGE.point.coins, 40);
assert.equal(L.EXCHANGE.ticket.coins, 160);
assert.match(indexSrc, /pickcoin/);
assert.match(indexSrc, /pickhundred/);
assert.match(indexSrc, /function pickPortraitCoin47/);
assert.match(indexSrc, /石阶缝里又滚出一枚/);
const exPoor = L.exchange({ coins: 10, shop40: L.normalizeOps({}), mainGod: { points: 0 } }, "shard", 1);
assert.equal(exPoor.ok, false);
const exSave = { coins: 400, shop40: L.normalizeOps({}), mainGod: { points: 2 } };
assert.equal(L.exchange(exSave, "shard", 1).ok, true);
assert.equal(exSave.coins, 380);
assert.equal(exSave.shop40.ops.shards, 1);
assert.equal(L.exchange(exSave, "point", 1).ok, true);
assert.equal(exSave.mainGod.points, 3);
assert.equal(L.exchange(exSave, "ticket", 1).ok, true);
assert.equal(exSave.shop40.ops.tickets, 1);
const ticketPull = L.pull({ coins: 0, shop40: L.normalizeOps({ ops: { tickets: 1 } }) }, 1, () => 0.5);
assert.equal(ticketPull.ok, true);
assert.equal(ticketPull.ticketUsed, 1);
assert.equal(ticketPull.tickets, 0);
assert.equal(L.snapshot(exSave).tickets, 1);
assert.equal(L.snapshot(exSave).picks, 0);
...
assert.match(gachaHost.innerHTML, /wishSeal47/);
