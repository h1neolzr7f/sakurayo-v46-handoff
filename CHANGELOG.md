...
当前版本 **v4.7.3**。完整条目按大版本写在 `docs/CHANGELOG_V*.md`。

## v4.7.3 — 2026-08-19

大厅立绘不再被呼吸层和 mask 裁掉。寻访页、揭示和大厅等级行写清入册叠伤。排除符按职业换成符纸 SVG，邮箱用精装信封/礼物图标。内测大额补给置顶可直接领。开发版 `4.7.3-yeying` / versionCode 6113。不包 `update`，存档键仍是 `sakurayoV3`。

## v4.7.2 — 2026-08-18

关掉玩家能误开的内测后门，立绘连点不再发币。内测补给改走邮箱。商店立绘和邮箱图标锁死尺寸，避免晚到脚本把界面撑大。开发版 `4.7.2-yeying` / versionCode 6112。不包 `update`，存档键仍是 `sakurayoV3`。

## v4.7.1 — 2026-08-18

主神大厅三丸默认可进，不再写第 4 章通关。寻访/核心/道具写入火力。档案补回成就图鉴。虚空怪不再请求不存在的 `_b` 帧。开发版 `4.7.1-yeying` / versionCode 6111。不包 `update`，存档键仍是 `sakurayoV3`。

## v4.7.0 — 2026-08-18

统一并行的夜樱修复线与大厅精装：触控穿透、口令观察者卡死、简报挡住暂停、橱窗刷新崩大厅、6109 图标精装、顶栏换角拆叠。开发版 `4.7.0-yeying` / versionCode 6110。不卖伤害，不包 `update`，存档键仍是 `sakurayoV3`。详见 `docs/CHANGELOG_V4.7.md` 和 `docs/REGRESSION.md`。
